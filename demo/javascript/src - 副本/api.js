...
module.exports = {
    ...
    addToChatRecord: function (msg, type) {
       ...
       localStorage.setItem(Demo.user||'', JSON.stringify(Demo.chatRecord));
        
    },

    releaseChatRecord: function (targetId) {
        var targetId = targetId || Demo.selected;
        if (localStorage.getItem(Demo.user)!=null)
            Demo.chatRecord=JSON.parse(localStorage.getItem(Demo.user));
        ...
        if(targetId){
            ...
            if(Demo.chatRecord[targetId] && Demo.chatRecord[targetId].messages){
                ...
                for(var i = 0 ; i < Demo.chatRecord[targetId].messages.length ; i++){
                    Demo.api.appendMsg(Demo.chatRecord[targetId].messages[i].message, Demo.chatRecord[targetId].messages[i].type,false);
                }
            }
        }
    },
    getNickName:function(username){
        var nickName=username;
        
        if (Demo.friends&&Demo.friends.length>0){
            
            for (var i=0;i<Demo.friends.length;i++){
                var friend=Demo.friends[i];
    ...
    appendMsg: function (msg, type,before) {
        if (!msg) {
            return;
        }
        //msg.chatTime=new Date().getTime();
        ...
        this.sentByMe = msg.from === Demo.user;
        if (msg.delay){
            var date2 = new Date(msg.delay);
            var localeString = date2.toLocaleString();
            msg.chatDate=localeString; 
        }
        if (msg.timestamp){

            var date2 = new Date(Number(msg.timestamp));
            var localeString = date2.toLocaleString();
            msg.chatDate=localeString; 
        }
        ...
        var brief = '',
            ...
            name = this.sentByMe ? Demo.user : msg.from,
            nickname = this.sentByMe ? Demo.userNickName : this.getNickName(name), //处理昵称
            targetId = this.sentByMe || msg.type !== 'chat' ? msg.to : msg.from,
            targetNode = document.getElementById('wrapper' + targetId),
            msgTime=msg.chatDate ,
