        ...
        if (WebIM.config.isWindowSDK) {
            ...
            conn.getRoster({
                success: function (roster) {
                    ...
                    for (var i in roster) {
                        var ros = roster[i];
                        var account=ros.name;
                        //默认昵称为账户
                        ros.nickName=ros.name;
                        
                        jq.ajax({url:'../cust/userAvatar?userCode='+account,async:false,success:function(result){
                            
                            if (result.data.userAvatar){
                                  var avatarUrl=result.data.userAvatar;
                                
                                  if (avatarUrl!='')
                                    ros.avatar=avatarUrl;

                            }
                            //有数据设置昵称
                            if (result.data.userName)
                                ros.nickName=result.data.userName;
        ...
        if(Demo.selected){
            ...
            switch(cate){
                case 'friends':
                    ...
                    props.nickName=Demo.selectedName;
