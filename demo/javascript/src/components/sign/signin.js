...
module.exports = React.createClass({
    ...
    login: function () {
        ...
        var type=false;
            ...
            Demo.loginCount++;
            if (Demo.loginCount>3){
                    this.props.loading('hide');
                    Demo.api.NotifyError('登录失败请稍后重试!');
                }else{
    ...
    signin: function (username, auth, type) {
        ...
        var options = {
            ...
            success: function (token) {
                ...
                var url = 'chat.action?username=' + encryptUsername;
               
               WebIM.utils.setCookie('webim_' + encryptUsername, token, 1);
                //window.history.pushState({}, 0, url);
            },
            error: function () {
               // window.history.pushState({}, 0, 'chat.action');
            }
        };
        ...
        Demo.user = username;
       /* if (username=='jliu'){
            Demo.userNickName='医生1';
        }
        if (username=='111111'){
            Demo.userNickName='患者1';
        }
        */
    ...
    componentDidMount: function () {
        ...
        if (WebIM.config.isAutoLogin&&!Demo.isLogin) {
    ...
    render: function () {
        ...
        return (
            <div className={this.props.show ? 'webim-sign' : 'webim-sign hide'}>
                ...
                <Input placeholder={Demo.lan.username} defaultFocus='true' type='hidden' ref='name' keydown={this.keyDown} value={username}  />
                <Input placeholder={Demo.lan.password} ref='auth' type='hidden' value={password} keydown={this.keyDown}/>
               
                <Button ref='button' text={Demo.lan.signIn} onClick={this.buttonclick}/>
                <p>{Demo.lan.tips}
                </p>
            </div>
        );
